...
let paiVoiceConfig: VoiceConfig;
try {
  ...
  paiVoiceConfig = config.voices.pai;
} catch (e) {
  // Fallback to hardcoded PAI voice config
  paiVoiceConfig = {
...
async function main() {
  ...
  // Send notification with voice (using PAI's voice from config)
  await sendNotification({
    title: 'PAI Context',
    message: message,
    voice_enabled: true,
    voice_name: paiVoiceConfig.voice_name,
    rate: paiVoiceConfig.rate_wpm,
    priority: 'low',
  });
